...
int main() {
    ...
    // Test sčítání
    int** matD = addMatrices(matA, matB, rowsA, colsB);
    std::cout << "\nVysledek A + B:" << std::endl;
    printMatrix(matD, rowsA, colsB);


    // Test odčítání
    int** matS = subtractMatrices(matA, matB, rowsA, colsB);
    std::cout << "\nVysledek A - B:" << std::endl;
    printMatrix(matS, rowsA, colsB);
