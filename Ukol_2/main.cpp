...
int** allocateMatrix(int rows, int cols) {
    int** matrix = new int*[rows]; // Alokace pole pointerů (řádky)
    for (int i = 0; i < rows; ++i) {
        matrix[i] = new int[cols]; // Alokace pro každý řádek (sloupce)
        for (int j = 0; j < cols; ++j) {
            matrix[i][j] = 0; // Inicializace prvků na 0
        }
    }
    return matrix;
}
...
void deallocateMatrix(int** matrix, int rows) {
    for (int i = 0; i < rows; ++i) {
        delete[] matrix[i]; // Uvolnění paměti pro každý řádek
    }
    delete[] matrix; // Uvolnění paměti pro pole pointerů
}
...
void printMatrix(int** matrix, int rows, int cols) {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            std::cout << std::setw(4) << matrix[i][j]; // Hezčí formátování
        }
        std::cout << std::endl;
    }
