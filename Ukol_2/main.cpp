...
int** allocateMatrix(int rows, int cols)
{
    ...
    // alokace arraye pointerů (rows)
    int** matrix = new int * [rows];
    // alokace arraye intu (columns) pro kazdy row
    for (int i = 0; i < rows; ++i)
    {
        matrix[i] = new int[cols];
        // inicializace kazdeho prvku na 0
...
{
    // uvolneni pameti pro kazdy row
    for (int i = 0; i < rows; i++)
    {
        delete[] matrix[i];
    }
    // uvolneni arraye pointeru
    delete[] matrix;
}
...
int main() {
    ...
    matA[2][0] = 69; matA[2][1] = -5; matA[2][2] = 0;
    ...
    matB[0][0] = 7; matB[0][1] = 0;
    matB[1][0] = 0; matB[1][1] = 1;
    matB[2][0] = 11; matB[2][1] = 0;
