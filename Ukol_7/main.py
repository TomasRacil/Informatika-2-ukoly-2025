...
class Vector2D:
    def __init__(self, x, y):
        ...
        if not isinstance(x, (int, float)) or not isinstance(y, (int, float)):
            raise TypeError("Souradnice vektoru musi byt cisla (int nebo float).")
...
if __name__ == "__main__":
    ...
    v1 = Vector2D(3, 4)
    v2 = Vector2D(5, 6)
    print(f"Můj vektor: {Vector2D.__str__(v1)}")
    print(f"Délka: {abs(v1)}")
    print(f"v1 + v2 = {Vector2D.__add__(v1, v2)}")
    print(f"v1 / 5 = {Vector2D.__sub__(v1, v2)}")
    print(f"porovnani: {Vector2D.__eq__(v1, v2)}")