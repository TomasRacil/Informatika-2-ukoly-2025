...
private:
    ...
    void allocateMatrix() {
        if (rows_ <= 0 || cols_ <= 0) {
            data_ = nullptr;
            return; // Ukončí funkci zde, pokud jsou rozměry neplatné
        }
        ...
        // Není potřeba druhá kontrola, ta první na začátku je dostatečná
    }
...
public:
    ...
    Matrix(int rows, int cols) {
        ...
        // TOTO JE OPRAVA:
        // Pokud je vstup záporný, nastavíme rozměr na 0.
        // Jinak použijeme danou hodnotu.
        rows_ = (rows < 0) ? 0 : rows;
        cols_ = (cols < 0) ? 0 : cols;
        
        // Poznámka: Odstraňte jakýkoli 'throw' z konstruktoru!

        // TODO: Zavolejte allocateMatrix()
        // Funkce allocateMatrix() si už správně poradí s rozměry 0xN, Nx0 nebo 0x0
        // a nastaví data_ = nullptr.
        allocateMatrix();
    }
    /**
    ...
    Matrix multiply(const Matrix& other) const {
        // Kontrola, zda jsou rozměry platné pro násobení
        if (cols_ != other.rows_) {
            return Matrix(0, 0); // Vrací prázdnou matici
        }   
        
        // Vytvoření nové matice 'result' (this->rows_ x other.cols_)
        Matrix result(rows_, other.cols_);

        // Provedení násobení matic (tři vnořené cykly)
        for (int i = 0; i < rows_; ++i) { // Řádky 'this' (a 'result')
            for (int j = 0; j < other.cols_; ++j) { // Sloupce 'other' (a 'result')
                
                int sum = 0; // Inicializace sumy pro prvek [i][j]
                
                // Vnitřní smyčka pro součet součinů
                for (int k = 0; k < cols_; ++k) { // Sloupce 'this' / Řádky 'other'
                    // sum += this->data_[i][k] * other.data_[k][j];
                    // Používáme gettery pro bezpečný přístup:
                    sum += this->getValue(i, k) * other.getValue(k, j);
                }
                
                result.setValue(i, j, sum); // Uložení výsledku
            }
        }
