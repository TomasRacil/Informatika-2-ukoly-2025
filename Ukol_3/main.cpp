...
public:
    ...
    int getValue(int row, int col) const {
        if (data_ == nullptr) throw std::out_of_range("Matrix has no data");
    ...
    Matrix add(const Matrix& other) const {
        if (rows_ != other.rows_ || cols_ != other.cols_) return Matrix(0, 0);
    ...
    Matrix multiply(const Matrix& other) const {
        if (cols_ != other.rows_) return Matrix(0, 0);
        if (data_ == nullptr || other.data_ == nullptr) return Matrix(0, 0);
