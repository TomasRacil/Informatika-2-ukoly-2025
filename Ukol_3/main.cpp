...
private:
    ...
    void allocateMatrix() {
        if (rows_ > 0 && cols_ > 0) {
            ...
            rows_ = 0; // Zajistíme konzistenci rozměrů
            cols_ = 0;
            data_ = nullptr;
        }
    }
...
public:
     ...
     */
    // Použití initializer list pro inicializaci členů a ošetření záporných rozměrů
    ...
    void print() const {
        ...
        for (int i = 0; i < rows_; ++i) {
            std::cout << "[";
            for (int j = 0; j < cols_; ++j) {
                // Přímý přístup je zde bezpečný, protože rozsah byl ošetřen dříve
    ...
    Matrix add(const Matrix& other) const {
        ...
        // Zde používáme přímý přístup k data_ pro výkon, protože rozměry byly ověřeny.
        for (int i = 0; i < rows_; ++i) {
            for (int j = 0; j < cols_; ++j) {
    ...
    Matrix multiply(const Matrix& other) const {
        if (cols_ != other.rows_) {
            std::cerr << "Error: Matrix dimensions invalid for multiplication (A.cols != B.rows)." << std::endl;
            return Matrix(0, 0);
        }
        ...
        for (int i = 0; i < rows_; ++i) {
            for (int j = 0; j < other.cols_; ++j) {
                ...
                for (int k = 0; k < cols_; ++k) {
                    sum += this->data_[i][k] * other.data_[k][j]; // Přímý přístup k data_
                }
